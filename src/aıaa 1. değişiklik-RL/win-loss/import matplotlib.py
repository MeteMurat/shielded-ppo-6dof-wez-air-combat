...
all_data = np.array(all_data)  # shape: (10, N, 3)
win = all_data[:, :, 0]
loss = all_data[:, :, 1]
tight = all_data[:, :, 2]

# Eğitim adımlarını ayarla (1000 adımda bir kayıt varsa)
steps = np.arange(win.shape[1]) * 1000  # Örn: [0, 1000, 2000, ..., N*1000]
...
ax1.set_xlabel('Train Steps for RL-based', fontsize=10)
ax1.set_ylabel('Number', fontsize=10)
